...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        self.goal_body = np.array([5.0, 5.0, -5.0])
        self.Kp = 0.5
    ...
    def goto_callback(self, msg):
        if msg.data:
            ...
            self.initial_position = self.odom_position.copy()
            self.get_logger().info("Objetivo relativo fijado: [5.0, 5.0, -5.0] en marco del dron")

    def odometry_callback(self, msg):
        self.odom_position = np.array([msg.position[0], msg.position[1], msg.position[2]])
    ...
    def cmdloop_callback(self):
        if self.offboardMode:
            ...
            if self.go_to_point and self.initial_position is not None:
                error = self.goal_body - (self.odom_position - self.initial_position)
                velocity_body = self.Kp * error
                self.get_logger().info(f"Pos actual body: {self.odom_position - self.initial_position}  Error: {error}  Velocidad: {velocity_body}")
            else:
